...
def fetch_river_data(f_inicio, f_fin, station_id):
    ...
    try:
        ...
        if not df.empty:
            ...
            # Detecta 'picos' aislados que pasen el filtro duro
            median = df['nivel'].median()
            std = df['nivel'].std()
            
            # Si hay suficiente variación para calcular estadísticas
            if pd.notna(std) and std > 0:
                # Marcamos como NaN cualquier valor a más de 4 desviaciones estándar de la mediana
